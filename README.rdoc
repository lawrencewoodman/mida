...
=== Inspecting an +Item+
Each +Item+ is a <tt>Mida::Item</tt> instance and has two main methods of
interest, +type+ and +properties+.

To find out the +itemtype+ of the +Item+:
  item = doc.items.first
  puts item.type

+properties+ are returned as a hash containing name/value pairs.  The
values can be either a single +String+ value, a <tt>Mida::Item</tt> instance
or an +Array+ containing either of the previous two value types.

To see the +properties+ of the +Item+:
  puts item.properties
